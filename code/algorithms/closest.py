...
Description:    This algorithm assigns each house to the closest battery 
...
class Closest:
    ...
    def run(self):
        ...
        # Runs while loop until all houses have been assigned or max_runs have been reached
        while len(self.district.houses) > 0:
            ...
            # Assgins house if compatible battery has been found
            if closest_bat != None:      
                closest_bat.add_house(house)
                create_cable(house, [closest_bat.row, closest_bat.column])
            
            # Recursively re-runs algorithm until valid solution has been found
    ...
    def return_valid(self) -> bool:
        ...
        if len(self.district.houses) != 0 or self.district == None:
            invalid_batteries = 0
            for battery in self.district.batteries:
                if battery.left_over_capacity < 0:
                    invalid_batteries += 1
            print(f"{len(self.district.houses)} not assigned. " + \
                f"{invalid_batteries} batteries have exceeded their capacity")
