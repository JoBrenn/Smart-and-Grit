...
def simulatedannealing_one_climb(district: District,
                                 iteration: int = 10000) -> None:
    """ Runs simulated annealing iteration one time
        and adds all costs to csv file
        Params:
    ...
    costs, costs_with_penalty\
        = simulatedannealing_add_costs_to_list(district, iteration)

    with open("results/simulatedannealing/simulatedannealing.csv",
              'w', newline='') as output_file, \
            open("results/simulatedannealing/simulatedannealing_penalty.csv",
...
def simulatedannealing_one_climb_graph_penalty():
    ...
    with open("results/simulatedannealing/simulatedannealing_penalty.csv",
              'r') as f:
        reader = csv.reader(f)
    ...
    for temp in range(3800, 4200, 100):
        ...
        for iteration_costs in all_costs:
            for iteration in zip(*iteration_costs):
                results.append((mean(iteration),
                                min(iteration), max(iteration)))

        # Add all iteration results to csv indicated with temperature
        with open(f"results/simulatedannealing/simulatedannealing_{temp}.csv",
                  'w', newline='') as f:
...
def simulatedannealing_temp_comparison_lowest_graph():
    ...
    ax.set_ylabel('Costs')
    fig.savefig(
                "results/simulatedannealing/simulatedannealing_temp_lowest.png"
               )
    plt.show()
...
def simulatedannealing_tuning(district: District, n: int = 100):
    ...
    with open("results/simulatedannealing/simulatedannealing_tuning.csv",
              'w', newline='') as f:
        result_writer = csv.writer(f, delimiter=',')
        for temp in range(3800, 4200, 100):
...
def run_simulatedannealing_experiments(district: District) -> None:
